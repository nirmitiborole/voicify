...
st.set_page_config(layout="wide", page_title = "Voicify")
tab1, tab2 = st.tabs(['OCR Text Extraction', 'Text to Audio Conversion'])

def file_save(content, file_name, folder_path, file_type='binary'):
    try:
...
with (tab1):
    st.title('OCR App')
    st.markdown("<h3 style='color: white;'>Extract text from imageand PDFs</h3>", unsafe_allow_html=True)
    ...
    if os.path.exists(output_folder):
        ...
        st.warning(f"Folder doesn't exist yet, but will be when saving files.")
    ...
    with col2:
        ...
        source_type = st.radio("Select source type", ("URL", "LocalUpload"), horizontal=True)
    ...
    if st.session_state["ocr_result"]:
        ...
        with col1:
            ...
            if file_type == "PDF":
                pdf_embed_html = f'<iframe src="{st.session_state
                ["preview_src"][idx]}" width="100%" height="400"frameborder = "0" > < / iframe > '
            ...
            with col2:
                ...
                # Show results in a text area that can be e
                ...
                with btn_col1:
                    # JSON download
                    json_data = json.dumps({"ocr_result":
                                                edited_text}, ensure_ascii=False, indent=2)
                ...
                with btn_col2:
                    # Text download
                    text_b64 = base64.b64encode(edited_text.encode('utf-8')).decode()
                ...
                with btn_col3:
                    # Save JSON to folder
                    if st.button(f"Save JSON to Folder",
                                 key=f"save_json_{idx}"):
                        json_data = json.dumps({"ocr_result":
                                                    edited_text}, ensure_ascii=False, indent=2)
                        success, result_path = file_save(json_data,
                        ...
                        if success:
                            ...
                            if os.path.exists(result_path):
                                ...
                                st.error(f"x File not found at{result_path}")
                        else:
                                st.error(f"Failed to save JSON: {result_path}")

                with btn_col4:
                    # Save Text to folder
                    if st.button(f"Save Text to Folder",
                                 key=f"save_text_{idx}"):
                        success, result_path = file_save(edited_text,
                            f"Output_{idx + 1}.txt",
                            output_folder,
                            file_type="text"
                        )
...
with tab2:
    ...
    if "output_folder" in locals():
        audio_output_folder = st.text_input("Output folder for saved files ",
        value = output_folder,
        help = "Files will be saved to this folder",
...
if os.path.exists(audio_output_folder):
    ...
    col1, col2 = st.columns(2)
    with col1:
        # Method selection - adjusted options since there's
        # tab
    ...
    if text_source == "OCR results":
        ...
        if st.session_state.get("ocr_result", []):
            result_options = [f"Result {i + 1}" for i in range(len(st.
                                                                   session_state["ocr_result"]))]
            default_idx = 0
            if "current_text_for_audio" in st.session_state:
                try:
                    default_idx = st.session_state["ocr_result"].index(st.session_state["current_text_for_audio"])
                        ...
                        default_idx = 0
            selected_result = st.selectbox("Select OCR result to convert",
                                           result_options,
                                           index=default_idx
                                           )
            ...
            text_for_audio = st.text_area("OCR text (you can edit before converting)",
        ...
        uploaded_text_file = st.file_uploader("Upload a tex",
                                              type=["txt", "md"])
    ...
    if "audib_results" not in st.session_state:
            ...
            with st.spinner("Converting text to audio..."):
                ...
                if success:
                    st.session_state["audio_results"].append({
                        "text": text_for_audio[:100] + "..." if len
                                                                (text_for_audio) > 100 else text_for_audio,
                        "path": audio_path,
    ...
    if st.session_state["audio_reslts"]:
        st.subheader("Generated Audio Files")
        for idx, audio_data in enumerate(st.session_state
                                         ["audio_results"]):
            with st.expander(f"Audio {idx + 1} - {audio_data['voice']}",
                ...
                with dl_col1:
                    ...
                    audio_b64 = base64.b64encode(audio_data
                                                 ["content"]).decode()
                    audio_href = f'<a href="data:audio/mp3;base64,{audio_b64}" download="Audio_{idx + 1}.mp3">DownloadAudioFile < / a > '
                    st.markdown(audio_href, unsafe_allow_html=True)

                with dl_col2:
                    # Save audio to folder
                    if st.button(f"Save Audio to Folder",
                                 key=f"save_audio_{idx}"):
